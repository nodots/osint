...
// Threat-scale calibration: percentiles of the pooled daily
// high-risk-pivotal counts across every baseline (one observation per
// cycle-day). Regenerated with the baselines so the scale always reflects
// the corrected replays it is defined against.
const pooled = Object.values(cycles)
  .flatMap((c) => c.series.map((p) => p.hrp))
  .sort((a, b) => a - b);
const pct = (q: number): number =>
  pooled[Math.min(pooled.length - 1, Math.ceil(q * pooled.length) - 1)]!;
const CAL_OUT = new URL(
  "../../../packages/elections-shared/src/threat-calibration.ts",
  import.meta.url,
).pathname;
writeFileSync(
  CAL_OUT,
...
export const THREAT_CALIBRATION = {
  ...
  methodologyVersion: "${out.methodologyVersion}",
...
console.log(
  `wrote ${CAL_OUT}: n=${pooled.length} p50=${pct(0.5)} p90=${pct(0.9)} max=${pooled[pooled.length - 1]}`,
);
