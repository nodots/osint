...
The threat-scale decision was made the same day: `controlThreat` is now
calibrated to percentiles of the pooled corrected-baseline cycle-days
(LOW < median 51, MODERATE ≥ median, HIGH ≥ p90 84), with the cutpoints
generated by `baseline:export` into
`packages/elections-shared/src/threat-calibration.ts`. Details in the
backtest report.
